...
Report_Type <- c("B.1.1","SA")

VOC <- c(B.1.1VOC,SAVOC)
#Provide variant positions
B.1.1pos <- factor(c(3037,14408,23403,28881,28882,28883))
SApos <- factor(c(1059,8782,14408,23403,25563,23063,23012))

make_report <- function(sample_name1,VOC){
  #sample_name1 <- filenames[1]
  report_type <- ifelse(VOC==B.1.1VOC,"B.1.1","SA")
  if(report_type=="B.1.1"){
  ...
  ggsave(grid, file=report_name, height = 10 , width = 12)
  
}

for (i in 1:length(filenames)){
  make_report(sample_name1=filenames[i],VOC=B.1.1VOC)
